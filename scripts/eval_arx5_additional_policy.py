...
@click.option("--additional_policy_port", default=None)
@click.option("--additional_policy_ip", default=None)
@click.option("--additional_ckpt_path", default=None)
@click.option(
    "--match_dataset",
    "-m",
    default=None,
...
def main(
    ...
    if additional_ckpt_path is not None:
        additional_cfg_path = additional_ckpt_path.replace(".ckpt", ".yaml")
        with open(additional_cfg_path, "r") as f:
            additional_cfg = OmegaConf.load(f)
        additional_obs_pose_repr = additional_cfg.task.pose_repr.obs_pose_repr
        additional_action_pose_repr = additional_cfg.task.pose_repr.action_pose_repr
        additional_obs_res = get_real_obs_resolution(additional_cfg.task.shape_meta)
    ...
    if additional_ckpt_path:
        additional_socket = context.socket(zmq.REQ)
        additional_socket.connect(f"tcp://{policy_ip}:{additional_policy_port}")
        use_additional_policy = True
    ...
    with SharedMemoryManager() as shm_manager:
        with Spacemouse(
            ...
            ####################################################
            if additional_ckpt_path is not None:
                additional_obs = copy.deepcopy(obs)
                additional_obs['camera0_rgb'] = additional_obs['camera0_rgb'][-cfg.task.shape_meta.obs.camera0_rgb.horizon:, :, :, :]
                additional_obs['robot0_eef_pos'] = additional_obs['robot0_eef_pos'][-cfg.task.shape_meta.obs.robot0_eef_pos.horizon:, :]
                additional_obs['robot0_eef_rot_axis_angle'] = additional_obs['robot0_eef_rot_axis_angle'][-cfg.task.shape_meta.obs.robot0_eef_rot_axis_angle.horizon:, :]

                additional_obs_dict_np = get_real_umi_obs_dict(
                    env_obs=additional_obs,
                    shape_meta=additional_cfg.task.shape_meta,
                    obs_pose_repr=additional_obs_pose_repr,
            ...
            if additional_ckpt_path:
                additional_socket.send_pyobj(additional_obs_dict_np)
                print(
                    f"    obs_dict_np sent to PolicyInferenceNode at tcp://{policy_ip}:{additional_policy_port}. Waiting for response."
                )
            start_time = time.monotonic()
            action = socket.recv_pyobj()
            if additional_ckpt_path:
                additional_action = additional_socket.recv_pyobj()
                additional_action = get_real_umi_action(additional_action, additional_obs, additional_action_pose_repr)
                ...
                while True:
                    ...
                    for key_stroke in press_events:
                        if key_stroke == KeyCode(char="q"):
                            ...
                            if additional_ckpt_path:
                                use_additional_policy = not use_additional_policy
                                print(f"use_additional_policy: {use_additional_policy}")
                ...
                try:
                    ...
                    while True:
                        ...
                        obs_data = {
                            ...
                            "obs": obs,
                            "additional_obs_dict_np": additional_obs_dict_np if additional_ckpt_path is not None else None,
                            "additional_obs_pose_repr": additional_obs_pose_repr if additional_ckpt_path is not None else None,
                            "additional_obs": additional_obs if additional_ckpt_path is not None else None,
                            "episode_start_pose": episode_start_pose,
                            "tx_robot1_robot0": tx_robot1_robot0,
                        }
                        np.save(
                        ...
                        if additional_ckpt_path:
                            additional_socket.send_pyobj(additional_obs_dict_np)
                            raw_additional_action = additional_socket.recv_pyobj()
                        ...
                        if additional_ckpt_path:
                            additional_action = get_real_umi_action(raw_additional_action, additional_obs, additional_action_pose_repr)
                        action_data = {
                            "action": action,
                            "additional_action": additional_action if additional_ckpt_path else None,
                            "raw_action": raw_action,
                            "raw_additional_action": raw_additional_action if additional_ckpt_path else None,
                        ...
                        this_target_poses = additional_action if additional_policy_port and use_additional_policy else action
